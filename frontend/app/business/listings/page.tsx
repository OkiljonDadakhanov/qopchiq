                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 mb-4 space-y-1">
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-1">
                        {listing.foodType && foodTypes[listing.foodType] ? (
                          <span className="inline-flex items-center gap-1 rounded-full border border-gray-200 px-2 py-1 text-xs text-gray-600">
                            <span>{foodTypes[listing.foodType].icon}</span>
                            <span>{foodTypes[listing.foodType].label}</span>
                          </span>
                        ) : null}
                        {(listing.dietaryPreferences ?? []).map((diet) =>
                          dietOptions[diet as keyof typeof dietOptions] ? (
                            <span
                              key={`${listing._id}-${diet}`}
                              className="inline-flex items-center gap-1 rounded-full border border-gray-200 px-2 py-1 text-xs text-gray-600"
                            >
                              <span>{dietOptions[diet as keyof typeof dietOptions].icon}</span>
                              <span>{dietOptions[diet as keyof typeof dietOptions].label}</span>
                            </span>
                          ) : null
                        )}
                      </div>

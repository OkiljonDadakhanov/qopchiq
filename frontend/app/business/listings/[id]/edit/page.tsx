...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      foodType: productData.foodType ?? null,
      dietaryPreferences: productData.dietaryPreferences ?? [],
          ...
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                {foodTypes.map((type) => {
                  const isActive = form.foodType === type.id
                  return (
                    <button
                      key={type.id}
                      type="button"
                      onClick={() =>
                        setForm((prev) => ({
                          ...prev,
                          foodType: prev.foodType === type.id ? null : type.id,
                        }))
                      }
                      className={`flex items-center justify-between rounded-lg border px-4 py-3 text-left transition ${
                        isActive ? "border-[#00B14F] bg-[#00B14F]/5" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      ...
                      <span className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                        isActive ? "border-[#00B14F] bg-[#00B14F] text-white" : "border-gray-300"
                      }`}
                      >
                        {isActive ? "✓" : ""}
                      </span>
            ...
            <div>
              ...
              <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-2">
                {dietOptions.map((diet) => {
                  const isSelected = form.dietaryPreferences.includes(diet.id)
                  return (
                    <button
                      key={diet.id}
                      type="button"
                      onClick={() =>
                        setForm((prev) => ({
                          ...
                          dietaryPreferences: isSelected
                            ? prev.dietaryPreferences.filter((item) => item !== diet.id)
                            : [...prev.dietaryPreferences, diet.id],
                        }))
                      }
                      className={`flex items-center justify-between rounded-lg border px-4 py-3 text-left transition ${
                        isSelected ? "border-[#00B14F] bg-[#00B14F]/5" : "border-gray-200 hover:border-gray-300"
                      }`}
                    >
                      <span className="flex items-center gap-3">
                      ...
                      <span className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                        isSelected ? "border-[#00B14F] bg-[#00B14F] text-white" : "border-gray-300"
                      }`}
                      >
                        {isSelected ? "✓" : ""}
                      </span>
